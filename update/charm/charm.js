// Charm update page specific code
...
async function setup() {
  ...
  let manifestResponse = await fetch('../../firmware/charm/manifest.json');
...
`<esp-web-install-button
  manifest="../../firmware/charm/manifest.json">
  <button slot="activate" class="button-36">update charm to v${version}</button>
</esp-web-install-button>`;
  ...
  // Load firmware files
  const firmwareFiles = [
    { name: 'bootloader.bin', url: '../../firmware/charm/bootloader.bin' },
    { name: 'partitions.bin', url: '../../firmware/charm/partitions.bin' },
    { name: 'boot_app0.bin', url: '../../firmware/charm/boot_app0.bin' },
    { name: 'firmware.bin', url: '../../firmware/charm/firmware.bin' }
  ];
}
